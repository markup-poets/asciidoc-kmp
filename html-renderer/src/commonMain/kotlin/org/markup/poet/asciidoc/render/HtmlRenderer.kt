...
/**
 * Main entry point for rendering an AsciiDoc ASG document to HTML.
 */
interface HtmlRenderer {
    /**
     * Renders an AsciiDoc ASG document to HTML.
     *
 ...
 *
 * Controls how the HTML renderer processes ASG nodes and generates output,
 ...
 * @param customRenderers Custom renderers keyed by node class simple name or block style
 ...
 * @param documentTitle Optional document title for <title> tag (overrides the document header title if provided)
 ...
 * 3. Uses AstVisitor to traverse and render the ASG
 ...
 *
 * @param blockRenderer The renderer for block nodes
 * @param inlineRenderer The renderer for inline nodes
...
class DefaultHtmlRenderer(
    ...
    private fun wrapInDocument(
        ...
        // Title: explicit config title, then the document header title
        val headerTitle = document.header?.title?.let { plainText(it) }?.takeIf { it.isNotBlank() }
        val title = options.documentTitle ?: headerTitle ?: "Document"
        builder.append("  <title>${escaper.escapeHtml(title)}</title>\n")

        // Metadata tags (if enabled)
        if (options.includeMetadata) {
            // Author
            document.attributes["author"]?.let { author ->
                builder.append("  <meta name=\"author\" content=\"${escaper.escapeAttribute(author)}\">\n")
            }

            // Description
            document.attributes["description"]?.let { description ->
                builder.append("  <meta name=\"description\" content=\"${escaper.escapeAttribute(description)}\">\n")
            }

            // Keywords
            document.attributes["keywords"]?.let { keywords ->
...
sealed class RenderException(message: String) : Exception(message) {
    /**
     * Thrown when the ASG structure is invalid or malformed.
     *
     * @param nodeName The name of the problematic node type
     * @param reason Description of why the ASG is invalid
     */
